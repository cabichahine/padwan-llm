...
class OutputModel(Protocol):
    """What `AgentSession.output` expects: the Pydantic v2 class surface, structurally.

    Any class with a `model_json_schema()` producing the JSON Schema of the answer and a
    `model_validate(obj)` returning an instance (or raising on a bad one) will do —
    `pydantic.BaseModel` subclasses qualify without this module importing Pydantic.
    """
...
class AgentSession:
    ...
    output: type[OutputModel] | None = None
    """Answer schema: a Pydantic-like class (see `OutputModel`). Enables `run()`."""
    ...
    _output_result: Any = field(init=False, default=_UNSET, repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        if self.output is not None:
            schema = dict(self.output.model_json_schema())
            schema.pop("title", None)  # the class name is noise for the model
            self._output_mcp_tool = McpTool(
                name=self.output_tool,
                description=(
                    "Submit the final answer. Call it exactly once, when the work is done."
                ),
                input_schema=schema,
    ...
    async def _handle_output(self, args: dict[str, Any]) -> Any:
        """The `output_tool` handler: validate, keep the instance, or ask for a repair.

        A failure past `max_repairs` is stored, not raised: `_dispatch_one` would turn an
        exception into a tool result and the loop would go on. `stream()` raises it after
        the round.
        """
        assert self.output is not None
        self._output_attempts += 1
        try:
            self._output_result = self.output.model_validate(args)
            ...
            return {
                "error": (
                    f"invalid answer, fix the fields below and call {self.output_tool} again"
                ),
                "details": str(exc),
            }
        return "accepted"
    ...
    async def stream(self, user_input: str | list[ContentPart]) -> AsyncIterator[str]:
        ...
        self._output_result, self._output_attempts, self._output_failure = (
            _UNSET,
            0,
            None,
        )
        ...
        while calls_remaining is None or calls_remaining > 0:
            ...
            if self._output_result is not _UNSET:
    ...
    async def run(self, user_input: str | list[ContentPart]) -> Any:
        ...
        text = await self.send(user_input)
        if self._output_result is not _UNSET:
            return self._output_result
        if text.startswith("(reached tool call limit"):
            raise OutputError(
